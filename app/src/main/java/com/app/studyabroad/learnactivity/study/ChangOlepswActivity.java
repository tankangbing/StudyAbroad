...
public class ChangOlepswActivity extends BaseActivity{
    ...
    private void initView() {
        setContentView(R.layout.activity_changolepsw);
...
//		achieveList = (ExpandableListView)findViewById(R.id.achieve_list);
        ...
        mContext = this;
        olePsw = (EditText) findViewById(R.id.cpsw_olepsw_edit);
...
//                        intent1.setClass(LoginActivity.this,DebugActivity.class);
    ...
    private boolean checkInput() {
        ...
        psw = olePsw.getText().toString();

        if (!SysUtil.isValidPassword(pwd1) || !SysUtil.isValidPassword(pwd2)||!SysUtil.isValidPassword(psw)) {
            Toast.makeText(ChangOlepswActivity.this,R.string.input_valid_password, Toast.LENGTH_SHORT).show();
            return false;
        }

        if (!StringUtils.equals(pwd1, pwd2)) {
            Toast.makeText(ChangOlepswActivity.this,R.string.two_password_different, Toast.LENGTH_SHORT).show();
            return false;
        }
...
//        });
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                map.put("stuid", appcache.getUserid());
                map.put("password", olePsw.getText());
                map.put("username", editPsw.getText());
//                map.put("type", loginType);
                map.put("mobileKey", MobileKey);
                Log.d("XX", mobileKey + " " + rsa);
                String url = FinalStr.ACCESSPRYPATH + "/edu3/app/urlto/user-u-psw.html";
                result = HttpUtil.getResultByPost(url,
                        map, ChangOlepswActivity.this, rsa);
                if (SysUtil.isBlank(result) || "NO_NET".equals(result)) {
//                    sendMessage(0,null);
                } else {
                    Log.d("ChangOlepswActivity", result);
...
//                            Toast.makeText(getApplicationContext(),R.string.password_Success,Toast.LENGTH_SHORT).show();
                    ...
                    } else if ("300".equals(maps.get("status") + "")) {//旧密码错误
                            bundle.putString("msg", maps.get("msg") + "");
    ...
    private void sendMessage(int str,Bundle b){
        ...
        ChangOlepswActivity.this.handler.sendMessage(msg);
    }
}